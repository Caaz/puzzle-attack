...
public class pAttack extends ApplicationAdapter {
  ...
  public int[] getGridPos(int x, int y) {
    ...
    pos[1] = (int) ((y - viewport.getTopGutterHeight() + (grid.shift * (screen[1] / 12))) / screen[1] * 12);
    pos[1] = -pos[1] + 12;
    return pos;
  }
    ...
    for (int y = 0; y < grid.tiles[0].length; y++) {
      for (int x = 0; x < grid.tiles.length; x++) {
        ...
        if (tile.active) {
          ...
          //color.a = 1-tile.cooldown;
          if (!(tile.flash > 0))
            color.sub(tile.cooldown, tile.cooldown, tile.cooldown, 0);
          else
            color.add(1 - tile.flash, 1 - tile.flash, 1 - tile.flash, 1);
          if (y < 2)
